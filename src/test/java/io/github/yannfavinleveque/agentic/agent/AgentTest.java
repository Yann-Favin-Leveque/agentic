...
class AgentTest {
    ...
    @Test
    void testAgentAllArgsConstructorWithBuilder() {
        // Use builder instead of all-args constructor for maintainability
        var agent = Agent.builder()
                .id("101")
                .name("Test Agent")
                .assistantIds(List.of("asst_openai_123", "asst_azure_1"))
                .model("gpt-4o")
                .instructions("Test instructions")
                .resultClass("com.example.TestResult")
                .temperature(0.7)
        ...
        assertEquals(4096, agent.getMaxTokens());
        assertEquals(true, agent.getWebSearch());
        assertEquals(false, agent.getCodeInterpreter());
